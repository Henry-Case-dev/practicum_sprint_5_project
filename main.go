...
func (r Running) Calories() float64 {
	...
	return ((CaloriesMeanSpeedMultiplier*avgSpeed + CaloriesMeanSpeedShift) * r.Weight / MInKm * r.Duration.Hours() * MinInHours)
}
...
func (w Walking) Calories() float64 {
	avgSpeed := w.meanSpeed() * KmHInMsec // Конвертация в м/с
	heightInM := w.Height / CmInM         // рост в метрах
	return ((CaloriesWeightMultiplier*w.Weight + (avgSpeed*avgSpeed/heightInM)*CaloriesSpeedHeightMultiplier*w.Weight) * w.Duration.Hours() * MinInHours)
}
...
func ReadData(training CaloriesCalculator) string {
	...
	info.Calories = calories // обновление калорий в структуре информации
